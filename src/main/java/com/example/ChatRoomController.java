...
public class ChatRoomController {
    ...
    @FXML
    private StackPane chatStateOverlay;
    @FXML
    private VBox chatLoadingPane;
    @FXML
    private ProgressIndicator chatLoadingIndicator;
    @FXML
    private Label chatLoadingLabel;
    @FXML
    private VBox chatErrorPane;
    @FXML
    ...
    @FXML
    private TextField searchMessages;
    // ── Right Sidebar ─────────────────────────────────────────────────────────
    @FXML
    ...
    @FXML
    private Button changeDOBtn;
    @FXML
    private ImageView userImage1;
    @FXML
    private Circle userImageClip1;
    // ── Emoji Picker ──────────────────────────────────────────────────────────
    @FXML
    ...
    private String earliestMessageTimestamp = null;
    private boolean isLoadingHistory = false;
    private HBox historyLoadingBox = null;
    ...
    private boolean ensureAudioPortReady() {
        if (audioCallService.isPortBound() && localAudioPort > 0)
            return true;
        try {
            localAudioPort = audioCallService.bindPort();
            if (SocketClient.isConnected()) {
    ...
    private boolean ensureVideoPortReady() {
        if (videoCallService.isPortBound() && localVideoPort > 0)
            return true;
        try {
            localVideoPort = videoCallService.bindPort();
            if (SocketClient.isConnected()) {
    ...
    private void onSignalingEvent(String line) {
        ...
        if (!line.startsWith("CALL_") && !line.startsWith("VIDEO_"))
            return;
        ...
        switch (parts[0]) {
            ...
            case "CALL_INVITE" -> {
                ...
                if (currentUserEmail == null)
                    return;
                if (Objects.equals(normEmail(from), normEmail(currentUserEmail)))
                    return;
    ...
    private void handleRealTimeNotification(String line) {
        ...
        if (type.equalsIgnoreCase("FRIEND_REQUEST")) {
            ...
            friendRequestsBtn.setStyle(
                    "-fx-border-color: #ff4d4f; -fx-border-width: 2; -fx-background-color: rgba(255,77,79,0.2);");
    ...
    private void initializeUI() {
        ...
        msgbox.heightProperty().addListener((obs, oldH, newH) -> {
            if (newH.doubleValue() > oldH.doubleValue()) {
                // Smart auto-scroll: only snap to bottom if previously at bottom or this is the
                // first bunch
                if (chatScroll.getVvalue() > 0.9 || oldH.doubleValue() == 0) {
                    chatScroll.setVvalue(1.0);
                }
            }
        });

        // Pagination listener removed as per "load history at once" request.
    }
    ...
    private void updateProfileMenuIcon() {
        ...
        if (userImage != null && userImage.getImage() != null) {
            ...
            try {
                profileIcon
                        .setImage(new Image(getClass().getResourceAsStream("/com/example/Image/default_avatar.png")));
            } catch (Exception ignored) {
            }
        }
        ...
        profileMenuBtn.setStyle(
                "-fx-background-color: rgba(123,44,255,0.15); -fx-background-radius: 19; -fx-border-color: rgba(123,44,255,0.25); -fx-border-radius: 19; -fx-border-width: 1;");
        profileMenuBtn.setPrefSize(40, 40);
    }

    private void loadCurrentUserInfo() {
        if (currentUserEmail == null)
            currentUserEmail = HomeController.currentEmail;
        if (currentUserName == null)
            currentUserName = HomeController.currentFirstName;
        if (currentUserName != null) {
            if (userIdLabel != null)
                userIdLabel.setText(currentUserName);
            if (USERNAME != null)
    ...
    public void setCurrentUserInfo(String email, String firstName) {
        ...
        if (userIdLabel != null)
            userIdLabel.setText(firstName);
        if (USERNAME != null)
            USERNAME.setText(firstName);
        updateProfileMenuIcon();
        // If controller ran initialize() before setCurrentUserInfo(), ensure calling is
        // initialized now.
    ...
    public void onAccountUpdate() {
        ...
        if (currentUserEmail != null)
            profilePictureCache.remove(normEmail(currentUserEmail));
        loadCurrentUserInfo();
        loadCurrentUserProfileImage(userImage, 40);
        loadCurrentUserProfileImage(userImage1, 80);
        updateProfileMenuIcon();
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                ...
                String data = response.startsWith("ALL_USERS|")
                        ? response.substring("ALL_USERS|".length())
                        : response;
                if (data.isBlank() || data.equals("EMPTY"))
                    return;
                for (String token : data.split(",")) {
                    if (!token.isBlank())
                        allUsers.add(token.trim());
                }
            });
        }).start();
    }
    ...
    public void loadFriendRequestBadge() {
        if (currentUserEmail == null || friendRequestBadge == null)
            return;
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                ...
                if (count > 0) {
                    friendRequestsBtn.setStyle(
                            "-fx-background-color: rgba(255,77,79,0.2); -fx-border-color: #ff4d4f; -fx-border-width: 2; -fx-border-radius: 19;");
                } else {
                    friendRequestsBtn.setStyle(
                            "-fx-background-color: rgba(123,44,255,0.15); -fx-border-color: rgba(123,44,255,0.25); -fx-border-radius: 19; -fx-border-width: 1;");
                }
            });
        }).start();
    }
    ...
    private void renderUserList(List<String> users, boolean isConversationList) {
        ...
        for (String u : users) {
            ...
            friendslist.getChildren()
                    .add(createUserItem(email, first, last, pic, isFriend, isSent, isConversationList));
        }
    }
    ...
    private VBox createUserItem(String email, String firstName, String lastName, String picBase64,
        ...
        if (!picBase64.isBlank()) {
            ...
            if (!profilePictureCache.containsKey(normEmail(email))) {
                try {
                    ...
                    if (!img.isError())
                        profilePictureCache.put(normEmail(email), img);
                } catch (Exception ignored) {
                }
            }
        } else {
        ...
        if (unreadCount > 0) {
            ...
            unreadBadge.setStyle(
                    "-fx-background-color: #ef4444; -fx-text-fill: white; -fx-background-radius: 10; -fx-padding: 2 6 2 6; -fx-font-size: 10px;");
        }
    ...
    private void sendFriendRequest(String targetEmail, Button button) {
        if (currentUserEmail == null || targetEmail == null || targetEmail.isBlank())
            return;
    ...
    private void applyMyGroupsResponse(String response) {
        ...
        String data = response.startsWith("MY_GROUPS|")
                ? response.substring("MY_GROUPS|".length())
                : response;
        if (!data.isBlank() && !data.equals("EMPTY")) {
            for (String token : data.split(",")) {
                if (!token.isBlank())
                    allGroups.add(token.trim());
            }
        }
        if (!showingUsersTab)
            renderGroupList(allGroups);
    ...
    private void renderGroupList(List<String> groups) {
        friendslist.getChildren().clear();
        if (groups.isEmpty()) {
            showEmptyLabel("No groups yet. Create one!");
            return;
        }
        for (String g : groups)
            friendslist.getChildren().add(createGroupItem(g));
    }
    ...
    private void selectChat(String userEmail, String userName, ImageView avatarView) {
        ...
        if (chatKey != null)
            loadProfilePictureForUser(chatKey, clientImage, 50);
    ...
    private void applyGroupInfoToHeader(String response) {
        ...
        if (clientIdLabel != null)
            clientIdLabel.setText("👥 " + name);
        if (clientImage != null) {
            if (!pic.isBlank())
                loadBase64Image(clientImage, pic, 50);
            else {
                clientImage.setImage(null);
    ...
    private void ensureGroupProfileShell() {
        if (groupProfilePane == null || groupProfileShellBuilt)
            return;
        ...
        seeMem.setOnAction(e -> {
            if (selectedGroupId != null)
                showGroupMembersDialog(selectedGroupId);
        });
    ...
    /**
     * Loads group name for the editor and refreshes the chat header (name +
     * picture).
     */
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                ...
                if (info != null && info.startsWith("GROUP_INFO|")) {
                    ...
                    int sep = inner.indexOf('|');
                    String name = sep >= 0 ? inner.substring(0, sep) : inner;
                    if (groupProfileNameField != null)
                        groupProfileNameField.setText(name);
    ...
    private void onGroupProfileSaveName() {
        if (selectedGroupId == null || currentUserEmail == null || groupProfileNameField == null)
            return;
    ...
    /**
     * Lists users from the DB (SQL LIKE on names/email), excludes current user and
     * existing members.
    ...
    private void showAddMembersToGroupDialog(String groupId) {
            ...
            new Thread(() -> {
                ...
                Platform.runLater(() -> {
                    ...
                    String payload = response.startsWith("GROUP_ADD_USERS|")
                            ? response.substring("GROUP_ADD_USERS|".length())
                            : response;
        ...
        dialog.showAndWait().ifPresent(bt -> {
            ...
            for (String email : selection.keySet()) {
                if (Boolean.TRUE.equals(selection.get(email)))
                    toAdd.add(email);
            }
            if (toAdd.isEmpty())
                return;
            ...
            new Thread(() -> {
                ...
                for (String m : toAdd) {
                    ...
                    if (ar == null || !ar.startsWith("MEMBER_ADDED"))
                        fail++;
                }
                int ff = fail;
                Platform.runLater(() -> {
                    loadGroupProfileData();
                    if (ff > 0)
                        showAlert(ff + " member(s) could not be added.");
                });
            }).start();
        });
    }
    ...
    private void loadPrivateChatHistoryInitial(int session) {
        if (selectedChatUserEmail == null || currentUserEmail == null)
            return;
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                if (session != chatSessionId || !Objects.equals(peer, selectedChatUserEmail))
                    return;
                ...
                // Automatically load the remaining history
                loadAllHistoryRecursive(session, false);
            });
        }).start();
    }

    private void pollPrivateMessages() {
        if (selectedChatUserEmail == null || currentUserEmail == null || isGroupChatActive)
            return;
        final int session = chatSessionId;
        final String peer = selectedChatUserEmail;
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                if (session != chatSessionId || !Objects.equals(peer, selectedChatUserEmail) || isGroupChatActive)
                    return;
                if (response == null || "CONNECTION_ERROR".equals(response) || response.isBlank())
                    return;
    ...
    private void applyPrivateHistoryFromResponse(String response, boolean replaceExisting) {
        ...
        // We do NOT reverse here because addAll(0, newNodes) will preserve 
        // the list's order (Oldest -> Newest) at the top of the VBox.

        List<Node> newNodes = new ArrayList<>();
        List<MessageData> newData = new ArrayList<>();
        ...
        for (String line : list) {
            ...
            HBox container = createMessageNode(text, isMe, ts, type, null);
            if (container == null)
                continue;

            if (replaceExisting) {
                msgbox.getChildren().add(container);
                allMessages.add(new MessageData(text, isMe, ts, type, container));
                if (lastMessageTimestamp == null || ts.compareTo(lastMessageTimestamp) > 0)
                    lastMessageTimestamp = ts;
                if (earliestMessageTimestamp == null || ts.compareTo(earliestMessageTimestamp) < 0)
                    earliestMessageTimestamp = ts;
            } else {
                newNodes.add(container);
                newData.add(new MessageData(text, isMe, ts, type, container));
                if (ts.compareTo(earliestMessageTimestamp) < 0)
                    earliestMessageTimestamp = ts;
            }
        }

        if (!replaceExisting && !newNodes.isEmpty()) {
            msgbox.getChildren().addAll(0, newNodes);
            allMessages.addAll(0, newData);
        }
    }
    ...
    private void loadGroupChatHistoryInitial(int session) {
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                if (session != chatSessionId || !Objects.equals(gid, selectedGroupId))
                    return;
                showChatLoading(false, null);
                if (response == null || "CONNECTION_ERROR".equals(response)) {
                ...
                // Automatically load the remaining history
                loadAllHistoryRecursive(session, true);
            });
        }).start();
    }

    private void pollGroupMessages() {
        if (selectedGroupId == null || !isGroupChatActive || currentUserEmail == null)
            return;
        final int session = chatSessionId;
        final String gid = selectedGroupId;
        final String since = lastMessageTimestamp;
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                if (session != chatSessionId || !Objects.equals(gid, selectedGroupId) || !isGroupChatActive)
                    return;
                if (response == null || "CONNECTION_ERROR".equals(response)
                        || response.isBlank() || "EMPTY".equals(response))
                    return;
                if ("NOT_MEMBER".equals(response))
                    return;
    ...
    private void applyGroupHistoryFromResponse(String response, boolean replaceExisting) {
        ...
        for (String line : list) {
            ...
            String sender = p[0];
            String text = p[1];
            String ts = p.length > 2 ? p[2] : "";
            boolean isMe = sender.equals(currentUserEmail);
            String label = isMe ? null : senderDisplayName(sender);
            String mtype = FileMessageCodec.isFileMessage(text) ? "file" : "text";

            HBox container = createMessageNode(text, isMe, ts, mtype, label);
            if (container == null)
                continue;

            if (replaceExisting) {
                msgbox.getChildren().add(container);
                allMessages.add(new MessageData(text, isMe, ts, mtype, container));
                if (lastMessageTimestamp == null || ts.compareTo(lastMessageTimestamp) > 0)
                    lastMessageTimestamp = ts;
                if (earliestMessageTimestamp == null || ts.compareTo(earliestMessageTimestamp) < 0)
                    earliestMessageTimestamp = ts;
                ...
                newNodes.add(container);
                newData.add(new MessageData(text, isMe, ts, mtype, container));
                if (earliestMessageTimestamp == null || ts.compareTo(earliestMessageTimestamp) < 0)
                    earliestMessageTimestamp = ts;
            }
        }
    ...
    private void displayMessage(String text, boolean isMe, String timestamp, String type) {
        displayMessage(text, isMe, timestamp, type, null, -1);
    }

    private HBox createMessageNode(String text, boolean isMe, String timestamp, String type, String groupSenderLabel) {
        if (FileMessageCodec.isFileMessage(text)) {
            try {
                ...
                if (parsed.mimeType().startsWith("audio/")) {
                    return createVoiceMessageNode(parsed, isMe, timestamp, groupSenderLabel);
                }
            } catch (Exception ignored) {
            }
            return createFileMessageNode(text, isMe, timestamp, groupSenderLabel);
        }
    ...
    private void displayMessageAtTop(String text, boolean isMe, String timestamp, String type,
            String groupSenderLabel) {
        HBox node = createMessageNode(text, isMe, timestamp, type, groupSenderLabel);
        if (node != null) {
            msgbox.getChildren().add(0, node);
            allMessages.add(0, new MessageData(text, isMe, timestamp, type, node));
        }
    }

    private void displayMessage(String text, boolean isMe, String timestamp, String type,
            String groupSenderLabel, int index) {
        HBox node = createMessageNode(text, isMe, timestamp, type, groupSenderLabel);
        if (node == null)
            return;
        if (index == 0) {
            ...
            msgbox.getChildren().add(node);
            allMessages.add(new MessageData(text, isMe, timestamp, type, node));
        }
    }

    private HBox createVoiceMessageNode(FileMessageCodec.Parsed parsed, boolean isMe, String timestamp,
            String groupSenderLabel) {
        HBox container = new HBox();
        container.setPadding(new Insets(4, 12, 4, 12));
        ...
        top.getChildren().addAll(playBtn, pb);

        Label tsLabel = new Label(timestamp);
        tsLabel.getStyleClass().add(isMe ? "message-timestamp-sent" : "message-timestamp-received");
        bubble.getChildren().addAll(top, tsLabel);
        container.getChildren().add(bubble);
    ...
    private void playAudioData(byte[] data, ProgressBar pb, Button playBtn) {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                while (finalClip.isRunning()) {
                    long currentMicros = finalClip.getMicrosecondPosition();
                    long durationMicros = finalClip.getMicrosecondLength();
                    double progress = durationMicros > 0 ? (double) currentMicros / durationMicros : 0;
                    Platform.runLater(() -> pb.setProgress(progress));
                    Thread.sleep(50);
                }

            } catch (Exception ex) {
                Platform.runLater(() -> showAlert("Playback Error: " + ex.getMessage()));
    ...
    private void displayVoiceMessage(FileMessageCodec.Parsed parsed, boolean isMe, String timestamp,
            String groupSenderLabel, int index) {
        HBox node = createVoiceMessageNode(parsed, isMe, timestamp, groupSenderLabel);
        if (index == 0) {
            msgbox.getChildren().add(0, node);
            allMessages.add(0, new MessageData("[Voice Message]", isMe, timestamp, "voice", node));
        } else {
            msgbox.getChildren().add(node);
            allMessages.add(new MessageData("[Voice Message]", isMe, timestamp, "voice", node));
        }
    }

    private HBox createFileMessageNode(String encoded, boolean isMe, String timestamp, String groupSenderLabel) {
        ...
        Label clip_node = new Label("📎 " + parsed.filename());
        clip_node.setWrapText(true);
        clip_node.getStyleClass().add("file-clip-label");
        ...
        Label tsLabel = new Label(timestamp);
        tsLabel.getStyleClass().add(isMe ? "message-timestamp-sent" : "message-timestamp-received");
        bubble.getChildren().addAll(clip_node, meta, saveBtn, tsLabel);
        if (isMe) {
            container.setAlignment(Pos.CENTER_RIGHT);
            bubble.setAlignment(Pos.CENTER_RIGHT);
            tsLabel.setAlignment(Pos.CENTER_RIGHT);
    ...
    private void displayFileMessage(String encoded, boolean isMe, String timestamp, String groupSenderLabel,
            int index) {
        HBox node = createFileMessageNode(encoded, isMe, timestamp, groupSenderLabel);
        if (node == null)
            return;
        if (index == 0) {
            msgbox.getChildren().add(0, node);
            allMessages.add(0, new MessageData(encoded, isMe, timestamp, "file", node));
        } else {
            msgbox.getChildren().add(node);
            allMessages.add(new MessageData(encoded, isMe, timestamp, "file", node));
        }
    }

    /**
     * Recursively loads all history until empty, triggered on initial chat entry.
     */
    private void loadAllHistoryRecursive(int session, boolean isGroup) {
        if (!isRunning || session != chatSessionId || earliestMessageTimestamp == null)
            return;

        new Thread(() -> {
            String cmd;
            if (isGroup) {
                cmd = "GET_GROUP_MESSAGES_BEFORE|" + selectedGroupId + "|" + earliestMessageTimestamp + "|"
                        + currentUserEmail + "|100";
            } else {
                cmd = "GET_MESSAGES_BEFORE|" + currentUserEmail + "|" + selectedChatUserEmail + "|"
                        + earliestMessageTimestamp + "|100";
            }

            String response = SocketClient.send(cmd);
            Platform.runLater(() -> {
                if (session != chatSessionId || !isRunning)
                    return;
                if (response != null && !response.equals("EMPTY") && !response.equals("ERROR")
                        && !response.equals("NOT_MEMBER") && !response.isBlank()) {
                    if (isGroup)
                        applyGroupHistoryFromResponse(response, false);
                    else
                        applyPrivateHistoryFromResponse(response, false);

                    // Recurse to load more until exhausted
                    loadAllHistoryRecursive(session, isGroup);
                }
            });
        }, "history-recursive-loader").start();
    }
    ...
    private void sendGroupMessage(String text) {
        if (selectedGroupId == null) {
            showAlert("Please select a group to chat with.");
            return;
        }
    ...
    public void cleanup() {
        ...
        if (callActive && activeCallId != null && selectedChatUserEmail != null) {
            ...
            if (videoCall) {
                SocketClient.sendPersistent(
                        "VIDEO_HANGUP|" + activeCallId + "|" + normEmail(currentUserEmail) + "|" + peer);
            } else {
                SocketClient
                        .sendPersistent("CALL_HANGUP|" + activeCallId + "|" + normEmail(currentUserEmail) + "|" + peer);
            }
        }
    ...
    private void initializeEmojiPicker() {
        ...
        for (String emoji : emojis) {
            ...
            btn.setOnMouseEntered(
                    e -> btn.setStyle("-fx-background-color: #e0d8f0; -fx-background-radius: 8; -fx-cursor: hand;"));
    ...
    private void onEmojiButtonClick() {
        emojipane_test.setVisible(!emojipane_test.isVisible());
    }

    @FXML
    private void onCloseEmojiPicker() {
    ...
    private void onGeminiSendClick() {
        ...
        if (geminiChatService == null) {
            appendGeminiMessage("⚠ AI service unavailable. Set the GEMINI_API_KEY environment variable and restart.",
                    "#8b3a3a");
            return;
        }
        if (currentUserEmail == null) {
    ...
    private void openCreateGroupDialog() {
            ...
            new Thread(() -> {
                ...
                Platform.runLater(() -> {
                    ...
                    String payload = response.startsWith("INVITE_USERS|")
                            ? response.substring("INVITE_USERS|".length())
                            : response;
        ...
        dialog.showAndWait().ifPresent(bt -> {
            ...
            for (Map.Entry<String, Boolean> ent : memberSelection.entrySet()) {
                if (Boolean.TRUE.equals(ent.getValue()))
                    picked.add(ent.getKey());
            }
    ...
    private void onVoiceMessageStop(MouseEvent event) {
        ...
        if (!voiceRecordService.isRecording())
            return;
        ...
        if (audio.length > 2000) { // Arbitrary threshold for valid voice message
            new Thread(() -> {
                try {
                    ...
                    if (isGroupChatActive)
                        sendGroupMessage(encoded);
                    else
                        sendPrivateMessage(encoded);
                } catch (Exception e) {
    ...
    @FXML
    private void onChangeDisplayPictureClick() {
        showAlert("Change profile picture coming soon!");
    }
    ...
    private void onAudioCallClick() {
        ...
        new Thread(() -> {
            ...
            // Include caller name so receiver sees a friendly name in the incoming-call
            // dialog.
    ...
    private void onVideoCallClick() {
        ...
        if (callActive && activeCallId != null) {
            String peer = normEmail(selectedChatUserEmail);
            SocketClient
                    .sendPersistent("VIDEO_HANGUP|" + activeCallId + "|" + normEmail(currentUserEmail) + "|" + peer);
            endActiveCallIfMatches(activeCallId);
            hideCallOverlay();
            return;
        }
    ...
    private void showCallingOverlay(String to, boolean video) {
        ...
        hangup.setStyle(
                "-fx-background-color: #e74c3c; -fx-text-fill: white; -fx-background-radius: 20; -fx-padding: 8 24; -fx-font-size: 14; -fx-cursor: hand;");
        hangup.setOnAction(e -> {
            if (activeCallId != null) {
                if (videoCall) {
                    SocketClient.sendPersistent(
                            "VIDEO_HANGUP|" + activeCallId + "|" + normEmail(currentUserEmail) + "|" + normEmail(to));
                } else {
                    SocketClient.sendPersistent(
                            "CALL_HANGUP|" + activeCallId + "|" + normEmail(currentUserEmail) + "|" + normEmail(to));
                }
            }
            endActiveCallIfMatches(activeCallId);
            hideCallOverlay();
    ...
    private void showIncomingCallOverlay(String callId, String from, String callerNameOverride, boolean video) {
        ...
        if (callActive) {
            if (video) {
                SocketClient.sendPersistent(
                        "VIDEO_REJECT|" + callId + "|" + normEmail(currentUserEmail) + "|" + normEmail(from) + "|BUSY");
            } else {
                SocketClient.sendPersistent(
                        "CALL_REJECT|" + callId + "|" + normEmail(currentUserEmail) + "|" + normEmail(from) + "|BUSY");
            }
            return;
        }
        ...
        accept.setStyle(
                "-fx-background-color: #2ecc71; -fx-text-fill: white; -fx-background-radius: 20; -fx-padding: 8 24; -fx-font-size: 14; -fx-cursor: hand;");
        accept.setOnAction(e -> {
            ...
            new Thread(() -> {
                if (video) {
                    ...
                    SocketClient.sendPersistent(
                            "VIDEO_ACCEPT|" + callId + "|" + normEmail(currentUserEmail) + "|" + normEmail(from));
                } else {
                    if (!ensureAudioPortReady()) {
                        Platform.runLater(() -> {
                            endActiveCallIfMatches(callId);
                    ...
                    SocketClient.sendPersistent(
                            "CALL_ACCEPT|" + callId + "|" + normEmail(currentUserEmail) + "|" + normEmail(from));
                }
        ...
        reject.setOnAction(e -> {
            ...
            if (currentUserEmail == null)
                return;
            if (video) {
                SocketClient.sendPersistent("VIDEO_REJECT|" + callId + "|" + normEmail(currentUserEmail) + "|"
                        + normEmail(from) + "|REJECTED");
            } else {
                SocketClient.sendPersistent("CALL_REJECT|" + callId + "|" + normEmail(currentUserEmail) + "|"
                        + normEmail(from) + "|REJECTED");
            }
            endActiveCallIfMatches(callId);
            hideCallOverlay();
    ...
    private void startAudioWithPeer(String callId, String peerIp, int peerPort) {
        ...
        if (!videoCall) {
            // Already shows calling/connecting overlay. If we are here, update it to
            // connecting.
        ...
        Platform.runLater(() -> {
            if (videoCall) {
                // Video overlay is already shown by startVideoWithPeer; just start the UI
                // monitor.
    ...
    private void startVideoWithPeer(String videoId, String peerIp, int peerPort) {
        if (activeCallId == null || !activeCallId.equals(videoId))
            return;
        videoCall = true;
        Platform.runLater(this::showConnectingOverlay);
        try {
    ...
    private void showActiveVideoCallOverlay() {
        ...
        // Bind remote view size to the callOverlay so it fills it regardless of window
        // size
    ...
    private void hideAllPopups() {
        if (profileSettingsPopup != null) {
            profileSettingsPopup.hide();
            profileSettingsPopup = null;
        }
        if (friendRequestsPopup != null) {
            friendRequestsPopup.hide();
            friendRequestsPopup = null;
        }

        if (ProfileSettingsController.currentPopup != null) {
            ProfileSettingsController.currentPopup.hide();
            ProfileSettingsController.currentPopup = null;
        }
        if (FriendRequestPopupController.currentPopup != null) {
            FriendRequestPopupController.currentPopup.hide();
            FriendRequestPopupController.currentPopup = null;
        }
        if (MyProfileController.currentPopup != null) {
            MyProfileController.currentPopup.hide();
            MyProfileController.currentPopup = null;
        }
        if (UpdateAccountController.currentPopup != null) {
            UpdateAccountController.currentPopup.hide();
            UpdateAccountController.currentPopup = null;
        }
    }

    private void showPopupAt(Node anchorNode, Popup popup) {
        if (anchorNode == null || popup == null)
            return;
        Window owner = null;
        try {
        ...
        if (popupWidth < 240)
            popupWidth = 240;
        if (popupWidth > 400)
            popupWidth = Math.min(popupWidth, 400);
    ...
    private void onGlobalSearchTyped() {
        if (globalSearch == null || currentUserEmail == null)
            return;
     ...
     * <li>Cache miss → shows default immediately, fetches GET_USER_INFO in
     * background,
    ...
    private void loadProfilePictureForUser(String email, ImageView imageView, int size) {
        ...
        new Thread(() -> {
            ...
            if (response == null || response.equals("ERROR") || response.equals("CONNECTION_ERROR"))
                return;
            String raw = response.startsWith("USER_INFO|")
                    ? response.substring("USER_INFO|".length())
                    : response;
     ...
     * searchMessages.setVisible(true);
     * searchMessages.requestFocus();
     * searchMessages.textProperty().addListener((obs, oldText, newText) ->
     ...
     * 
     * if (matchCount > 0) {
     * System.out.println("[Search] " + matchCount + " messages found for: \"" +
     * query + "\"");
     * } else {
     ...
     * if (highlight) {
     * container.
     * setStyle("-fx-background-color: rgba(255, 255, 0, 0.3); -fx-background-radius: 8;"
     * );
     * } else {
    ...
    private void scrollToBottom() {
        // Force scroll to bottom (e.g. initial load or user sent message)
        Platform.runLater(() -> Platform.runLater(() -> chatScroll.setVvalue(1.0)));
    }

    private void scrollToBottomSmart() {
        // Only scroll to bottom if user is already at/near the bottom (Sticky Scroll)
        Platform.runLater(() -> {
            if (chatScroll.getVvalue() > 0.95 || chatScroll.getVvalue() < 0.0) { // <0 handle initial/empty
                scrollToBottom();
            }
        });
    }
    ...
    private Stage getChatOwnerStage() {
        if (createGroupBtn != null && createGroupBtn.getScene() != null
                && createGroupBtn.getScene().getWindow() instanceof Stage s)
            return s;
        if (msgField != null && msgField.getScene() != null
                && msgField.getScene().getWindow() instanceof Stage s)
            return s;
        if (leftbase != null && leftbase.getScene() != null
                && leftbase.getScene().getWindow() instanceof Stage s)
            return s;
        return null;
    }
}